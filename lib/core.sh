    ...
    ksh)
	   ...
	   sh) echo "sh : Not compatible une bash"
		     exit 1
			  ;;
     *)
        echo "unknown : warning maybe not fully functionall"
        ;;
esac
	  ...
	  then 
			...
			if [[ -z "${_variable_to_check}" ]]
				then
				  if [[ -z "${_on_fail_action}" ]]
					  then
							set_message "EdEMessage" "0" "empty or undefined" 
							${_on_fail_action}
						else 
...
{
	#|# Var to set  :
	#|# _path_to_test : The path of the directory to test
	#|# ${1}         : First parameter, used to set _path_to_test
  ...
  if [[ ${_test_mode} -eq 1 ]]
	...
	if [[ ${_internal_call} -eq 1 ]]
	   then 
				set_message "check" "0" "Checking directory: [ ${_path_to_test} ] "
	fi 
...
function get_directory_list()
{
  ...
  do_empty_var_control "${Base_Param_Dir_To_TEST}"   "Base_Param_Dir_To_TEST"  "2" "1" "0"
	do_empty_var_control "${Result_Var_Name}"          "Result_Var_Name"         "2" "1" "0"  

  do_directory_test "${Base_Param_Dir_To_TEST}"
        
  for _dirs_listed in $(ls -1 ${Base_param_Dir}/)
    do 
      if [ -d "${Base_param_Dir}/${_dirs_listed}" ]
        then 
          set_message "debug" "0" "Directory found: [ ${_dirs_listed} ] "
          Sendback_Dir_Listed="${Sendback_Dir_Listed} ${_dirs_listed}"
        else 
          set_message "debug" "0" "Found: [ ${_dirs_listed} ] but it is not a directory"
      fi 
  done 
...
function do_directory_test() 
{
  #|# Description :
  #|# This function validates the existence of a directory at a given path.
  #|# It is designed to be reusable in both standalone checks and internal
  #|# validation flows within the scripting framework.
  #|#
  #|# The function integrates with the messaging and error-handling system
  #|# (set_message / do_empty_var_control) and supports conditional verbosity
  #|# and error management depending on the execution context.
  #|#
  #|# Parameters :
  #|# ${1} : Base_Param_Dir_To_TEST
  #|#        Absolute or relative path of the directory to test (Mandatory)
  #|#
  #|# ${2} : _internal_call
  #|#        Define the verbosity behavior (Optional)
  #|#        1 → Display check and success messages
  #|#        0 → Silent mode (used for internal chaining)
  #|#
  #|# ${3} : _test_mode
  #|#        Control error handling strategy (Optional)
  #|#        1 → Test mode (no blocking error, errLevel=0)
  #|#        0 → Standard mode (blocking error, errLevel=8)
  #|#
  #|# Variables :
  #|# Base_Param_Dir_To_TEST : Directory path to validate
  #|# _internal_call         : Control message display behavior
  #|# _test_mode             : Control error handling level
  #|# _errLevel              : Computed error level based on mode
  #|#
  #|# Behavior :
  #|# - Validates that the input parameter is not empty
  #|# - Optionally logs a "check" message when called externally
  #|# - Tests if the directory exists using [-d]
  #|# - Returns a success or error message via the messaging framework
  #|# - Adapts error severity depending on execution mode
  #|#
  #|# Return / Output :
  #|# - No direct return value
  #|# - Emits standardized messages (EdSMessage / EdEMessage)
  #|# - Can trigger controlled exit depending on _errLevel (external logic)
  #|#
  #|# Usage :
  #|# do_directory_test "/path/to/directory"
  #|# do_directory_test "/path/to/directory" "1" "0"
  #|#
  #|# Notes :
  #|# - Designed to be compliant with structured logging and audit requirements
  #|# - Intended for reuse in validation chains (pre-check, install, control phases)
  #|# - Relies on external functions:
  #|#     → do_empty_var_control
  #|#     → set_message
  #|#
  #|# Security / Compliance :
  #|# - Prevents execution with undefined or empty input
  #|# - Ensures deterministic validation behavior
  #|# - Compatible with DevSecOps pipelines and audit traceability
  ############ STACK_TRACE_BUILDER #####################
  Function_Name="${FUNCNAME[0]}"
  Function_PATH="${Function_PATH}/${Function_Name}"
  ######################################################
  ...
  if [[ ${_test_mode} -eq 1 ]]
	...
	if [[ ${_internal_call} -eq 1 ]]
	  then 
			set_message "check" "0" "Checking directory: [ ${Base_Param_Dir_To_TEST} ] "
	fi
	# Check if the directory exists
	if [ -d "${Base_Param_Dir_To_TEST}" ]
		then 
		  if [[ ${_internal_call} -eq 1 ]]
			  then 
					set_message "EdSMessage" "0" "FOUND" 
			fi 
		else 
			set_message "EdEMessage" "1" "Not FOUND" 
	fi 
...
function do_load_file() 
{
  ...
  do_empty_var_control "${_file_to_source}"       "_file_to_source"       "2" "1" "0"
...
{
	#|# Description :
	#|# This function updates the APT package index on Debian-based systems.
	#|# It ensures that the local package catalog is synchronized with the
	#|# configured repositories before any installation or upgrade operation.
	#|#
	#|# Behavior :
	#|# - Emits a debug message with the current function execution path.
	#|# - Emits a check message indicating that the package catalog update is starting.
	#|# - Verifies if the update has already been executed during the current runtime
	#|#   using the internal flag `_apt_update_runed`.
	#|#     → If already executed:
	#|#         - Emits a success message indicating the operation is skipped.
	#|#     → If not executed:
	#|#         - Runs `apt-get update` silently (stdout/stderr redirected).
	#|#         - Validates the return code via `do_error_control`.
	#|#         - Sets `_apt_update_runed="1"` to prevent redundant executions.
	#|#
	#|# Variables :
	#|#     _apt_update_runed : Internal state flag (0/1)
	#|#                         Prevents multiple executions of apt update
	#|#                         within the same script lifecycle.
	#|#
	#|# Dependencies :
	#|#     - sudo
	#|#     - apt-get
	#|#     - set_message (logging framework)
	#|#     - do_error_control (error handling framework)
	#|#
	#|# Return :
	#|#     No direct output.
	#|#     Relies on `do_error_control` for exit handling in case of failure.
	#|#
	#|# Notes :
	#|# - Output is intentionally silenced for clean logging.
	#|# - Designed to be idempotent within a single execution context.
	#|# - Must be executed prior to any package installation to ensure consistency.
	############ STACK_TRACE_BUILDER #####################
	Function_PATH="${Function_PATH}/${FUNCNAME[0]}"
	######################################################
  set_message "debug" "0" "current function path : [ ${Function_PATH} ]  | function Name [ ${Function_Name} ] "

  set_message "check" "0" "Package Catalogue update"
...
function do_apt_install_package ()
{
  ...
  if [ "${aptPackageStatus}" = "NOT INSTALLED" ]
		then 
	    set_message "check" "0" "installing package : [ ${_package_name} ]" 
...
function do_apt_uninstall_package ()
{
  ...
  if [ "${aptPackageStatus}" = "INSTALLED" ]
		then 
	    set_message "check" "0" "uninstalling package : [ ${_package_name} ]" 
...
function Test_apt_package_presence ()
{
	...
	set_message "check" "0" "searching for installed package : [ ${_package_name} ] " 
...
function do_gitea_create_secret()
{
	...
	#|#
	#|# The function performs input validation using do_empty_var_control before
	...
	#|# - do_empty_var_control function must be defined
	#|#
	local _secret_name="${1}"
  local _secret_value="${2}"
   
	do_empty_var_control "${_secret_name}"       "_secret_name"       "2" "1" "0"
	do_empty_var_control "${_secret_value}"      "_secret_value"      "2" "1" "0"
  do_empty_var_control "${GITEA_ORG_or_USER}"  "GITEA_ORG_or_USER"  "2" "1" "0"
	do_empty_var_control "${GITEA_REPO}"         "GITEA_REPO"         "2" "1" "0"
  do_empty_var_control "${GITEA_URL}"          "GITEA_URL"          "2" "1" "0"
...
function display_continue_or_exit
{
   #|# Description :
   #|# This function displays an interactive menu prompting the user to either
   #|# continue execution or exit the script. It is typically used as a control
   #|# gate before executing sensitive operations (e.g., actions requiring elevated
   #|# privileges or impacting system state).
   #|#
   #|# The function leverages the internal messaging framework (set_message) and
   #|# provides traceability via system logging when critical choices are made.
   #|#
   #|# Parameters :
   #|# ${1} : BASE_MSG_FOR_CHOICE
   #|#        Message displayed before presenting the menu (warning context)
   #|#
   #|# ${2} : BASE_MSG_FOR_MENU
   #|#        Message used as menu prompt (PS3 variable)
   #|#
   #|# Variables :
   #|# BASE_MSG_FOR_CHOICE : Warning message displayed to the user
   #|# BASE_MSG_FOR_MENU   : Menu prompt message
   #|# Result              : User selection from the menu
   #|# My_Ident            : Current user UID (used for audit logging)
   #|#
   #|# Behavior :
   #|# - Displays a warning message before presenting choices
   #|# - Prompts the user with a menu using Bash 'select'
   #|# - Handles two main actions:
   #|#     1 → Continue execution (logs user identity for audit)
   #|#     2 → Exit request (emits error message)
   #|# - Handles invalid input by triggering an error message
   #|#
   #|# Return / Output :
   #|# - No explicit return value
   #|# - Emits structured messages via set_message
   #|# - Writes an audit log entry using 'logger' when continuing
   #|#
   #|# Usage :
   #|# display_continue_or_exit "Warning message" "Select an option: "
   #|#
   #|# Notes :
   #|# - Designed for interactive scripts only (requires user input)
   #|# - The loop continues until a valid selection is made
   #|# - Does not directly exit the script; exit behavior must be handled externally
   #|#
   #|# Security / Compliance :
   #|# - Provides traceability via syslog when sensitive continuation is chosen
   #|# - Helps enforce user acknowledgment before critical operations
   #|# - Compatible with audit and controlled execution requirements
   ############ STACK_TRACE_BUILDER #####################
   Function_Name="${FUNCNAME[0]}"
   Function_PATH="${Function_PATH}/${Function_Name}"
   ######################################################
   set_message "Warning1" "${BASE_MSG_FOR_CHOICE}"

   PS3="${Base_Var_Menu_Message}"
   select Result in Continue EXIT
   do
      case ${Result} in
            "1") set_message "Warning1" "User select to user root account"
               My_Ident="$(id -u)"
               logger " $( date )  scripting user whith root account by [ ${My_Ident} ] "
               ;;
            "2") set_message "EdEMessage" "1" "User select to exit" 
               ;;
            *) set_message "EdEMessage" "1" "Bad input exit" 
               ;;
      esac
   done
